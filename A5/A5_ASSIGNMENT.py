...
#phi in an iterative method using the Laplace Equation
...
for k in range(Niter):
    ...
    phi[ii] =1  # restore the condition of Potential =1 inside circle
...
"""
Computes the Current density J from the equations
"""
...
ax.quiver(X[1:-1,1:-1],Y[1:-1,1:-1],Jx,Jy, scale=7)
ax.scatter(ii[1]-(Nx-1)/2,ii[0]-(Ny-1)/2,marker = 'o', color ='r')
plt.xlabel(r'+ $x\longrightarrow$')
plt.ylabel(r'+ $y\longrightarrow$')
